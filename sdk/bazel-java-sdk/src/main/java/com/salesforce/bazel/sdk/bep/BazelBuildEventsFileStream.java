...
/**
 * Bazel build event protocol stream (BEP) for a Bazel workspace. A BEP stream allows you to monitor and react 
 * to build events emitted from Bazel.
 * <p>
 * Because this stream integrates with your builds with Bazel using BEP, this stream will work with command line
 * builds but also with IDE builds.
 * <p>
 * After creation, you must start the stream by calling activateStream().
 * <p>
 * <b>NOTE:</b>
 * This feature requires a configuration change to your Bazel workspace: you must add the following lines to
...
public class BazelBuildEventsFileStream extends BazelBuildEventStream {
    ...
    /**
     * Bazel supports output of BEP events to one or more files. For each BEP file you
     * have configured for your workspace, add it here.
     * The file does not need to exist. This helps in cases when the build has not yet run
     * and the file has not been written yet. But this does mean that passing an incorrect
     * File object will cause your events to be missed.
     * <p>
     * You must decide if the stream should parse the state of the BEP file when this
     * stream is created. This might be correct behavior, but think about cases in which
     * the user has not run the build in many days. The BEP file would contain build events
     * from many days ago. If it would be odd for your app to react to build events that are 
     * many days old, set <i>parseOnStart</i> to false.
     */
    ...
    /**
     * The stream will monitor the BEP json files for any changes, based on a polling
     * interval. 
     */
            ...
            while (true) {
                ...
                for (MonitoredFile monitoredFile : monitoredFiles) {
                    if (!monitoredFile.file.exists()) {
                        ...
                        System.out.println("File ["+monitoredFile.file.getAbsolutePath()+"] does not exist yet. Run a build?");
                        continue;
                    }
                    ...
                    if (currentLastMod == monitoredFile.fileLastModifiedMS) {
                        ...
                        System.out.println("FilePoller will not parse "+monitoredFile.file.getName()+" because it hasn't changed.");
                        continue;
                    }
                    ...
                    BazelBuildEventsFileContents newContent = bepFile.readEvents("BazelBuildEventsFileStream", 
                        previousContent);
