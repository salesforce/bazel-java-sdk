...
public class BEPTargetCompletedEventTest {
    ...
    public void testSuccessEventParse() throws Exception {
        ...
        assertEquals("file:///private/var/tmp/_bazel_mbenioff/8fc74f66fda297c82a847368ee50d6a4/execroot/myrepo/bazel-out/darwin-fastbuild/bin/foo/foo.jar", uri.getUri().toString());
        assertEquals("bin", uri.getPrefixes().get(2));
        uri = event.getImportantOutput().get(1);
        assertEquals("foo/foo", uri.getId());
        assertEquals("file:///private/var/tmp/_bazel_mbenioff/8fc74f66fda297c82a847368ee50d6a4/execroot/myrepo/bazel-out/darwin-fastbuild/bin/foo/foo", uri.getUri().toString());
        assertEquals("bin", uri.getPrefixes().get(2));
    }
